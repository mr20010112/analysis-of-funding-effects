...
def DID(new_df, path):
    ...
    model = smf.ols('paper_num ~ treatment*year', data=new_df).fit()
    return model

def DID_after(new_df, path):
    new_df = pd.read_csv(path, dtype={'id': np.int32, 'year': str, 'treatment': np.int32, 'after': np.int32})
    model = smf.ols('paper_num ~ treatment*after', data=new_df).fit()
    return model
...
def train_and_contrast(train_id, contrast_id):
    ...
    paper_num, Sum_JCR_Q1, Sum_ZKY, Avg_IF, Sum_IF, Max_IF, Sum_JCR, Avg_JCR, Max_JCR = get_index(contrast_id)
...
# =============================================================================
#     # ======================== modify ===============================
#     update_paper_num = paper_num.copy()
#     for i, col in enumerate(paper_num):
#         if len(col)==0:
#             update_paper_num[i]=[0]*11
#         for j, row in enumerate(col):
#             if np.isnan(paper_num[i][j]):
#                 pass
#                 # update_Sum_IF[i][j] = 0
#       # ======================== modify ===============================
# =============================================================================
         
    df_contrast = create_df_by_index_all(df_contrast, paper_num, year, contrast_id) #paper_num
...
if __name__ == "__main__":
    ...
    #contrast_id = ['56428568600', '56465472100', '56393916300', '56415466200', '56411541500', '56402494700', '56547842900', '56585167100', '56496116000', '56518653800', '56505881100', '56517174700', '56110096100', '56113608200', '56125489700', '56118040000']
